...
public class TrackballInteractor {
    ...
    public TrackballInteractor(int width, int height) {
        ...
        trackballXformPlane[0] = 1.0;
        trackballXformPlane[5] = 1.0;
        trackballXformPlane[10] = 1.0;
        trackballXformPlane[15] = 1.0;
    }
    ...
    public double[] getTransformationMatrixPlane() {
        return trackballXformPlane;
    }
    ...
    public void setLeftMouse(boolean leftMouse) {
    ...
    public void updateTransform(GL2 gl, boolean leftMouse) {
        gl.glPushMatrix();
        gl.glLoadIdentity();
        if (leftMouse) {
            ...
            gl.glRotated(angle, axis[0], axis[1], axis[2]);
            gl.glMultMatrixd(trackballXformPlane, 0);
            gl.glGetDoublev(GL2.GL_MODELVIEW_MATRIX, trackballXformPlane, 0);
        }
        gl.glPopMatrix();
        setRotating(false);
    }
